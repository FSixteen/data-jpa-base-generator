...
import io.github.fsixteen.data.jpa.base.generator.annotations.plugins.FilterIn.List;

/**
 * 过滤后成员判断快捷注解.
 *
 * <p>
 * 该注解是 {@link Membership} 的 in 语义快捷包装, 并允许在参与谓词前对集合元素做过滤.
 * 零配置时默认等价于“当前字段路径 IN 当前字段运行时集合值”.
 * </p>
 *
 * @author FSixteen
 * @since 1.0.1
...
@Documented
@Membership(op = CompareOp.IN, collection = @CollectionPolicy())
@Inherited
public @interface FilterIn {
    ...
    /**
     * Defines several {@link FilterIn} annotations on the same element.
     *
     * @see FilterIn
     */
    @Target({ FIELD, METHOD })
