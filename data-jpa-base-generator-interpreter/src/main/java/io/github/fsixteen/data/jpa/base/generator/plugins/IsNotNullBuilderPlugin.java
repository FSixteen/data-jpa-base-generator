 ...
 * @since 1.0.2
 */
public class IsNotNullBuilderPlugin extends AbstractComputerBuilderPlugin<IsNotNull> {

    private static final Logger LOG = LoggerFactory.getLogger(IsNotNullBuilderPlugin.class);

    @Override
    @SuppressWarnings("rawtypes")
    public ComputerDescriptor<IsNotNull> toPredicate(AnnotationDescriptor<IsNotNull> ad, Object obj, Root<?> root, AbstractQuery<?> query, CriteriaBuilder cb)
    ...
    protected Predicate createPredicate(CriteriaBuilder cb, Optional<Expression> leftExpression, Optional<Expression> rightExpression) {
        return leftExpression.get().isNotNull();
    }
